...
class Habits {
	...
	static async logComplete(completedForm) {
		await db.query(`insert into completed_habits (habit_id, completed_count) values ($1, $2);`, [completedForm.id, completedForm.completedCount]);
	}
	...
	static async logMissed(missedForm) {
		await db.query(`insert into missed_habits (habit_id, missed_count) values ($1, $2);`, [missedForm.id, missedForm.missedCount]);
	}
	...
	static async editHabit(form) {
		await db.query(
			`update habits set habit_name = $1, frequency = $2, period = $3, start_date = $4, end_date = $5, temp_start_date = $6, temp_end_date = $7 where id = $8`,
			[form.habitName, form.frequency, form.period, form.startDate, form.endDate, form.tempStartDate, form.tempEndDate, form.id]
		);
	}
